...
import type { TDicomUploaderQueuePayload } from '~common/types';
...
export class DicomUploaderQueue implements QueueInterface {
  ...
  public async processItem(_: DbService, payload: string) {
    const { item, isPhiExists } = JSON.parse(payload) as TDicomUploaderQueuePayload;
    const uploader = this.getDicomUploader();
    await uploader.uploadPacsFile(item, isPhiExists);
