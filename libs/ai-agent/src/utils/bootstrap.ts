...
import type { AiAgentPathService } from '~ai-agent/services';
import { CronService } from '~core/cron';
import { LogService } from '~core/log';
import { QueueService } from '~core/queue';

export async function bootstrap(module: any, aiAgentPathService: AiAgentPathService, httpPort: number) {
  const appServer = await NestFactory.create(
    module,
    new ExpressAdapter(express()),
    { logger: new LogService(aiAgentPathService, 'NEST') },
  );
